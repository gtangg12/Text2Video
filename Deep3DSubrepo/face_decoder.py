...
class Face3D():
	...
	def Reconstruction_Block(self,coeff,opt):
		...
		world_coords, render_imgs,img_mask,img_mask_crop = self.Render_block(face_shape_t,norm_r,face_color,camera_scale,f_scale,self.facemodel,opt.batch_size,opt.is_train)
		
		translation_inv = -translation
		rotation_inv = tf.linalg.inv(rotation)
		#frontalized_world_coords = self.Rigid_transform_block(world_coords, rotation_inv, translation_inv)
		...
		#self.frontalized_world_coords = frontalized_world_coords
	...
	def Rigid_transform_block(self,face_shape,rotation,translation):
		...
		face_shape_t = face_shape_r + translation[:, None, :]
	...
	def Render_block(self,face_shape,face_norm,face_color,camera_scale,f_scale,facemodel,batchsize,is_train=True):
		...
		with tf.device('/cpu:0'):
			vertex_ids, barycentric_coordinates, img_rgba = mesh_renderer.mesh_renderer(face_shape,
		...
		if is_train:
			...
			with tf.device('/cpu:0'):
				vertex_ids, barycentric_coordinates, img_crop_rgba = mesh_renderer.mesh_renderer(mask_face_shape,
		...
		# calculate approx of coords in 3d based on avg of verticies
		# change to barycentric based interpolation if insufficient
		vi_0, vi_1, vi_2 = tf.unstack(tf.reshape(vertex_ids, [-1, 3]), axis=1)

		#face_shape bs*N vertex*3
		#vi_0 shape (bs*244*244)
		face_flattened = tf.reshape(face_shape, (-1, 3))
		v_0 = tf.gather(face_flattened, vi_0)
		v_1 = tf.gather(face_flattened, vi_1)
		v_2 = tf.gather(face_flattened, vi_2)

		v = (tf.multiply(barycentric_coordinates[:, 0:1], v_0) +
		 	 tf.multiply(barycentric_coordinates[:, 1:2], v_1) +
			 tf.multiply(barycentric_coordinates[:, 2:3], v_2))
		#v = (v_0 + v_1 + v_2) / 3.0

		v = tf.reshape(v, (batchsize, -1, 3))

		if not self.reference:			
			v = tf.subtract(v, self.translation[:, None, :])
			v = tf.matmul(v, tf.matrix_inverse(self.rotation)) #could also try to use transposing for better numerical stability

		v = tf.reshape(v, [batchsize, 224, 224, 3])
		v = v[:, ::-1, :, :] #flipping vertically to convert spatial coordinates to image coordinates

		return v,img_rgba,mask,mask
